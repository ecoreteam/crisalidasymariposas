...
<div class="gf-post-formats-ui-tabs" id="gf-post-formats-ui-tabs">
	...
	<ul class="tab-nav clearfix">
		...
		foreach ($post_format_views as $post_format) {
			...
			if ($post_format == 'standard') {
				$format_string = esc_html__('Standard','spring-plant');
	...
	<div class="tab-content">
		<?php
		foreach ($post_format_views as $post_format) {
			if (!in_array($post_format,$post_formats) || in_array($post_format, array('standard', 'quote'))) continue;
			...
			<div class="postbox tab-pane<?php echo esc_attr($class); ?>" id="tab-<?php echo esc_attr($format_hash);?>">
				<?php Spring_Plant_Post_Formats_UI()->get_template_part("format-{$post_format}");?>
			</div>
			<?php
		}
		?>
