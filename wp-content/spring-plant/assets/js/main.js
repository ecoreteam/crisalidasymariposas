(function ($) {
    ...
    G5_Main.page = {
		...
		initInstagram: function () {
			$('.instagram-5-columns .instagram-pics').addClass('owl-carousel').owlCarousel({
				margin: 0,
				dots: false,
				nav: false,
                rtl: isRTL,
    ...
    G5_Main.popup = {
        ...
        showMailchimpPopup: function (popupWrapper) {
            if (popupWrapper.length) {
                popupWrapper.modal();
                var checkbox = $('#remember-show', popupWrapper),
                    close = $('button.close', popupWrapper);
                $(document).on('click', function (event) {
                    check_and_set_cookie($(event.target).closest('#gsf-popup-mailchimp-form').length === 0 && checkbox.attr('checked') === 'checked');
                });
                close.on('click', function () {
                    check_and_set_cookie(checkbox.attr('checked') === 'checked');
                });
            ...
            function check_and_set_cookie($condition) {
            	if($condition) {
