...
?>
    ...
    <div id="we-get-social" class="postbox">
        ...
        <div class="clear-fix">
			...
			<a href="https://www.webempresa.com/wordpress/como-mejorar-velocidad-wordpress.html" target="_blank" ><h1 style="font-weight:700;" >Sigue mejorando la velocidad de tu WordPress haciendo clic aquí »<h1></a>
...
-webkit-box-shadow: 10px 7px 60px -10px rgba(0,0,0,0.58);
-moz-box-shadow: 10px 7px 60px -10px rgba(0,0,0,0.58);
box-shadow: 10px 7px 60px -10px rgba(0,0,0,0.58);
...
<h2 class="nav-tab-wrapper">
    <a href="?page=we-img-optimizer&tab=display_resumen" class="nav-tab <?php echo ( $active_tab == 'display_resumen' || $active_tab == '' ) ? 'nav-tab-active' : ''; ?> ">Inicio</a>
		...
		if ( isset($_POST["submitoptall"] ) && isset( $_POST["weaction"]) && $_POST["weaction"] == "optimizeall" ) {
			...
			*/
			...
			if ($plannedImgsUniq > 0 ) {
				...
				printf( '<div id="message" class="updated fade" style="padding:15px;" ><span style="font-size:20px;">' . "Planificada la optimización de $plannedImgsUniq imágenes" . "</span></div>" );

			}else {
				printf( '<div id="message" class="updated fade" style="padding:15px;" ><span style="font-size:20px;">' . "No hay imágenes que optimizar, están todas optimizadas o planificadas para su optimziación" . "</span></div>" );
	...
	?>
				...
				<center>
					<form id="weautoptimize_form_optall" method="post" name="weautoptimize_optall">
						...
						<input class="webigbutton" value="Optimizar todas las imágenes" name="submitoptall" type="submit" ></input>
					</form>
				</center>
				<br style="clear:both;margin-top:50px;" />	
	<?php
